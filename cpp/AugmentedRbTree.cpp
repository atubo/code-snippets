...
public:
  void Print() const;
  //RedBlackTreeNode();
  RedBlackTreeNode(int a_, int id_): a(a_), id(id_), max(a), size(1) {}
  void update();
  int a;
  int id;
  int max;
...
public:
  ...
  ~RedBlackTree();
  void Print() const;
  RedBlackTreeNode *Insert(int a, int c, int id);
  ...
  stack<RedBlackTreeNode *> Enumerate() ;
...
protected:
  ...
  void TreeInsertHelp(RedBlackTreeNode *, int);
...
void RedBlackTreeNode::update()
{
	max  = std::max(a, std::max(left->max, right->max));
...
{
  nil = new RedBlackTreeNode(MIN_INT, -1);
  nil->left = nil->right = nil->parent = nil;
  nil->red = 0;
  nil->max = MIN_INT;
  nil->size = 0;
  
  root = new RedBlackTreeNode(MAX_INT, -1);
...
void RedBlackTree::TreeInsertHelp(RedBlackTreeNode* z, int c) {
...
RedBlackTreeNode * RedBlackTree::Insert(int a, int c, int id)
{
  RedBlackTreeNode * y;
  RedBlackTreeNode * x;
  RedBlackTreeNode * newNode;

  x = new RedBlackTreeNode(a, id);
  TreeInsertHelp(x, c);
  newNode = x;
  x->red=1;
...
void RedBlackTreeNode::Print() const {
#if 0
  storedEntry->Print();
  printf(", key=%i ",key);
  printf("  l->key=");
  ...
  printf("  red=%i\n",red);
#endif
  printf("id %d a %d max %d size %d left id %d right id %d\n", 
		 id, a, max, size, left->id, right->id);
}
...
stack<RedBlackTreeNode *> RedBlackTree::Enumerate()  {
  stack<RedBlackTreeNode *> enumResultStack;
