...
public:
    ...
    void testSubtreeSize() {
        LinkCutTree lct(5);
        for (int i = 1; i <= 5; i++) {
            lct.val[i] = 1; // doesn't matter
            lct.makeRoot(i);
        }
        for (int i = 1; i <= 5; i++) {
            CPPUNIT_ASSERT_EQUAL(1, lct.size[i]);
        }
        ...
        lct.makeRoot(3);
        lct.makeRoot(4);
        vector<int> expected = {0, 2, 1, 2, 3, 1};
        for (int i = 1; i <= 5; i++) {
            CPPUNIT_ASSERT_EQUAL(expected[i], lct.size[i]);
        }
        lct.link(3, 4);
        expected = {0, 2, 1, 2, 5, 1};
        for (int i = 1; i <= 5; i++) {
            CPPUNIT_ASSERT_EQUAL(expected[i], lct.size[i]);
        }
