...
class SuffixNode 
{
    public:
        ...
        SuffixNode(int begin, int end, int depth, SuffixNode *parent) 
        {
            ...
            for (int i = 0; i < 38; i++) children[i] = NULL;
            this->begin = begin;
            this->end = end;
            this->parent = parent;
            this->depth = depth;
...
#include <cstdio>
void printNode(SuffixNode* node, int recur) {
    printf("recur=%d ", recur);
    if (node) {
        printf("depth=%d begin=%d end=%d\n",
               node->depth, node->begin, node->end);
        for (int i = 0; i < 38; i++) {
            if (node->children[i]) {
                printf("i=%d ", i);
                printNode(node->children[i], recur+1);
            }
        }
    }
}
